...
3. CHỈ ĐÁNH DẤU LÀ ĐẠO VĂN (isDuplicate: true) NẾU PHÁT HIỆN MỘT ĐOẠN VĂN DÀI (Ít nhất 2-3 câu liên tục) ĐƯỢC SAO CHÉP Y HỆT từ một tài liệu cụ thể. BỎ QUA hoàn toàn các định nghĩa, lý luận chung chung, các câu tiêu đề, mục tiêu giáo dục thông thường, tên đề tài, các cụm từ chuyên môn sư phạm quen thuộc. TUYỆT ĐỐI KHÔNG BẮT LỖI TRÙNG LẶP CHO CÁC CÂU NGẮN mang tính chất giới thiệu hoặc mục đích chung.
4. TÌM RA THÔNG TIN CỤ THỂ của các tài liệu gốc (Tên học liệu, phần nội dung của tài liệu gốc bị Đạo văn dán vào \`matched_snippet\`). Chỉ trích dẫn \`matched_snippet\` nếu nó thực sự dài và sao chép y hệt.
...
6. Phát hiện AI: Dựa vào văn phong, cấu trúc câu đặc thù của AI (như đoạn mở đầu/kết luận khuôn mẫu, ngôn ngữ trung lập quá mức, v.v.) để ước lượng tỷ lệ % nội dung được viết bởi AI (thường các đoạn văn chung chung, lan man không có dữ kiện thực tế).
